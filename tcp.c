...
static int errno_is_temporary(int e) {
    if (e == EINTR || e == EWOULDBLOCK || e == EAGAIN || e == EINPROGRESS ||
        e == EALREADY || e == EISCONN) {
        return 1;
    }
    else {
        return 0;
...
void string_from_ip_address(const unsigned char *bytes, char *str) {
    sprintf(str, "%u.%u.%u.%u", (unsigned)bytes[0], (unsigned)bytes[1],
...
static int tcp_internal_sleep(struct tcp *tcp, struct timeval *timeout,
                              int reading, int writing) {
    int result;
    fd_set rfds, wfds;
    ...
    while (1) {
        ...
        if (result > 0) {
            if (reading && FD_ISSET(tcp->fd, &rfds))
                return 1;
            if (writing && FD_ISSET(tcp->fd, &wfds))
                return 1;
        }
        else if (result == 0) {
            return 0;
...
struct tcp *tcp_accept(struct tcp *master, time_t stoptime) {
    ...
    while (1) {
        if (!tcp_sleep(master, stoptime, 1, 0))
            goto failure;
...
int tcp_fd(struct tcp *tcp) { return tcp->fd; }
